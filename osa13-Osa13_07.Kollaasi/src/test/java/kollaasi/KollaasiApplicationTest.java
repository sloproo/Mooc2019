...
public class KollaasiApplicationTest extends ApplicationTest {
    ...
    public void start(Stage stage) throws Exception {
        KollaasiSovellus sovellus = new KollaasiSovellus();

        try {
            Application app = Application.class.cast(sovellus);
        } catch (Throwable t) {
            fail("Periihän luokka KollaasiSovellus luokan Application.");
        }

        try {
            Reflex.reflect(KollaasiSovellus.class).method("start").returningVoid().taking(Stage.class).invokeOn(sovellus, stage);
        } catch (Throwable ex) {
            fail("Onhan luokalla KollaasiSovellus metodi start, joka saa parametrina Stage-olion. Jos on, tarkista että metodi toimii. Virhe: " + ex.getMessage());
        }
    ...
    @Test
    public void stagessaPaneJossaImageView() {
        getImageView();
    }
    ...
    public void osa1() {
        ...
        for (String kohtaJaVari : osan1PikselienOdotetutArvot()) {
            String[] arr = kohtaJaVari.split("\t");
            ...
            fail("Kulmaan kopiointi ei toiminut odotetusti. Kohdassa " + arr[0] + ", " + arr[1] + " ollut arvo ei ollut odotettu.");
        }
    }
    ...
    public void osa2() {
        ...
        for (String kohtaJaVari : osan2PikselienOdotetutArvot()) {
            String[] arr = kohtaJaVari.split("\t");
            Color c = pixelReader.getColor(Integer.parseInt(arr[0]), Integer.parseInt(arr[1]));
            ...
            fail("Pienen kuvan kopiointi neljään kulmaan ei toiminut odotetusti. Kohdassa " + arr[0] + ", " + arr[1] + " ollut arvo ei ollut odotettu.");
        }
    }
    ...
    public void osa3() {
        ...
        arvot.addAll(osan1PikselienOdotetutArvot());
        arvot.addAll(osan2PikselienOdotetutArvot());

        for (String kohtaJaVari : arvot) {
            String[] arr = kohtaJaVari.split("\t");
            Color c = pixelReader.getColor(Integer.parseInt(arr[0]), Integer.parseInt(arr[1]));
            ...
            fail("Negatiivin muodostaminen ei toiminut odotetusti. Kohdassa " + arr[0] + ", " + arr[1] + " ollut arvo ei ollut odotettu.");
        }
    }
    ...
    private ImageView getImageView() {
        ...
        assertNotNull("Ensimmäistä näkymää vastaavalle Scene-oliolle tulee antaa parametrina käyttöliittymäkomponenttien asetteluun tarkoitettu olio (tässä Pane). Nyt Scene-oliolta ei löytynyt komponentteja sisältävää oliota.", elementtienJuuri);

        Pane asettelu = null;
        try {
            asettelu = Pane.class.cast(elementtienJuuri);
        } catch (Throwable t) {
            fail("Käytäthän ensimmäisen näkymän käyttöliittymäkomponenttien asetteluun Pane-luokkaa.");
        }

        assertNotNull("Ensimmäisestä näkymästä vastuussa olevalle Scene-oliolle tulee antaa parametrina käyttöliittymäkomponenttien asetteluun tarkoitettu Pane-olio.", asettelu);
        assertFalse("Pane-oliolla pitäisi olla yksi lapsi, joka on tyyppiä ImageView. Nyt lapsia ei ollut lainkaan.", asettelu.getChildren().isEmpty());
        assertTrue("Pane-oliolla pitäisi olla yksi lapsi, joka on tyyppiä ImageView. Nyt lapsia oli ", asettelu.getChildren().size() == 1);

        assertTrue("Pane-oliolla pitäisi olla yksi lapsi, joka on tyyppiä ImageView.", asettelu.getChildren().get(0).getClass().isAssignableFrom(ImageView.class));

        return (ImageView) asettelu.getChildren().get(0);
    }

    // TODO: jatka testejä
    private List<String> osan1PikselienOdotetutArvot() {
    ...
    private List<String> osan2PikselienOdotetutArvot() {
